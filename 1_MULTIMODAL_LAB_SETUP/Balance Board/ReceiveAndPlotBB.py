...
#color plot
channeltoshow = [1,2,3,4]
                #blue     #orange     #green        #cyan
colortoshow = [(0,0,255),(225,140,0),(34,139,34),(0,255,255)]
...
class DataInlet(Inlet):
    ...
    def __init__(self, info: pylsl.StreamInfo, plt: pg.PlotItem):
        ...
        self.curves = [pg.PlotCurveItem(x=empty, y=empty, autoDownsample=True, pen=pg.mkPen(color = COLOR, width=1)) for _,COLOR in zip(range(self.channel_count), colortoshow)] 
        channelit = 0 #note we are going to only show some channels that are specified above
        for curve in self.curves:
            channelit=channelit+1
            if channelit in channeltoshow:
                plt.addItem(curve)
...
def main():
    ...
    streams = pylsl.resolve_stream('name', 'BalanceBoard_stream')
    
    # Create the pyqtgraph window
    pw = pg.plot(title='Balanceboard sensor stream')
    plt = pw.getPlotItem()
